...
import com.dwtedx.income.addrecord.adapter.IncomeRecyclerViewAdatper;
...
public class IncomeFragment extends BaseFragment implements RecordKeyboardView.OnKeyboardClickListener {
    ...
    private IncomeRecyclerViewAdatper adapter;
    private ImageView mCheckImageView;
    private TextView mCheckTextView;
    private int mCheckTypeId = 27;
...
//      mRecyclerView.setItemAnimator(new DefaultItemAnimator());//设置item的添加删除动画，采用默认的动画效果
        adapter = new IncomeRecyclerViewAdatper(getContext(), mDiTypeList);
        mRecyclerView.setAdapter(adapter);//设置Adapter
        adapter.setOnItemClickListener(new IncomeRecyclerViewAdatper.OnItemClickListener() {//添加监听器
            @Override
            public void onItemClick(View view, int postion) {
                if(CommonConstants.INCOME_ROLE_ADD_TYPE == mDiTypeList.get(postion).getType()){
                    Intent intent = new Intent(getContext(), IncomeTypeActivity.class);
    ...
    public void onResume() {
        super.onResume();
        mDiTypeList.clear();
        mDiTypeList.addAll(DITypeService.getInstance(getContext()).findAll(CommonConstants.INCOME_ROLE_INCOME));
    ...
    private void saveIncome(){
        ...
        DiIncome income = new DiIncome(
                0, null, 0, CommonConstants.INCOME_ROLE_INCOME, money,
