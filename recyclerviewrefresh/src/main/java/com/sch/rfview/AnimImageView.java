...
public class AnimImageView extends ImageView {
    ...
    public AnimImageView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if(screenWidth > 720) {
            textWidth = mPaint.measureText(text) + 73;
        }else if(screenWidth > 480){
            textWidth = mPaint.measureText(text);
        }else{
            textWidth = mPaint.measureText(text) - 22;
        }
    ...
    public void draw(Canvas canvas) {
        ...
        if (getLayoutParams().height > dp1) {
            ...
            // 文本
            if (height >= maxHeight) {
                canvas.drawText("释放开始同步账本", centerX - textWidth, 90, mPaintText);
                canvas.drawText("释放开始同步账本", centerX - textWidth, 90, mPaintText);
            } else {
                canvas.drawText("继续下拉同步账本", centerX - textWidth, 90, mPaintText);
                canvas.drawText("继续下拉同步账本", centerX - textWidth, 90, mPaintText);
            }
